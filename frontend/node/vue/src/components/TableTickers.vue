    ...
    <div class='d-flex justify-center mt-2 flex-nowrap align-items-end' style='flex-basis: 30%'>
      <table class='table-sm table-bordered'>
        ...
        <caption style='caption-side: top'><small></small></caption>
          <tr>
            <td class='bg-dark text-light'>day</td>
          </tr>
          <tr>
            <td class='bg-dark text-light' style='border-top: 2px solid'>week</td>
          </tr>
          <tr>
            <td class='bg-dark text-light text-start' style='border-top: 2px solid' nowrap><input type='radio' id='r20' name='period' value="20">month</td>
          </tr>
          <tr>
            <td class='bg-dark text-light text-start' style='border-top: 2px solid' nowrap><input type='radio' id='r60' name='period' value="60">quarter</td>
          </tr>
          <tr>
            <td class='bg-dark text-light text-start' style='border-top: 2px solid' nowrap><input type='radio' id='r125' name='period' value="125" checked="checked">half-year</td>
          </tr>
          <tr>
            <td class='bg-dark text-light text-start' style='border-top: 2px solid' nowrap><input type='radio' id='r250' name='period' value="250">year</td>
          </tr>
          <tr>
            <td class='bg-dark text-light text-start' style='border-top: 2px solid' nowrap><input type='radio' id='r1000' name='period' value="1000">4 years</td>
          </tr>
          <tr>
            <td class='bg-dark text-light text-start' style='border-top: 2px solid' nowrap><input type='radio' id='r5000' name='period' value="5000">20 years</td>
          </tr>
        <!--/form-->
      </table>
      <table class='table-sm table-bordered'>
      ...
      <table class='table-sm'>
        ...
        <tr v-for="ticker, key in tickers[0]"  v-bind:key='key'>
          <td class='change-vix text-right' style='border-right: 2px solid black'>
            {{ ticker.dif_vix }}
          </td>
          ...
          <td class='change-tnx text-right'>
            {{ ticker.dif_tnx }}
          </td>
        </tr>
      </table>
...
export default {
  ...
  updated: function() { 
    ...
    tr = this.$el.querySelectorAll(".change-tnx");
    arr = Array.from(tr);
    ...
    for (let j = 0; j < arrInt.length; j++) {
      if (arrInt[j] > 0) {
        arr[j].style.backgroundColor = 'rgba(255, 193, 7,'  + arrInt[j]/maxArr + ')';
      }
      else if (arrInt[j] < 0) {
        arr[j].style.backgroundColor = 'rgba(23, 162, 184,'  + arrInt[j]/minArr+ ')';
      }
    }
    
    tr = this.$el.querySelectorAll(".change-vix");
    arr = Array.from(tr);

    arrInt = arr.map((i) => parseFloat(i.innerText));
    maxArr = Math.max.apply(null, arrInt);
    minArr = Math.min.apply(null, arrInt);
    for (let j = 0; j < arrInt.length; j++) {
      if (arrInt[j] > 0) {
        arr[j].style.backgroundColor = 'rgba(255, 193, 7,'  + arrInt[j]/maxArr + ')';
      }
